import { HttpService } from '@nestjs/axios';
import {
    AxiosRequestConfig,
    AxiosResponse,
    AxiosError,
    AxiosInstance,
    default as axios,
} from 'axios';
import {
    HttpException,
    HttpStatus,
    Injectable,
    Logger,
} from '@nestjs/common';
import { catchError, map, retry } from 'rxjs/operators';
import { firstValueFrom, timer } from 'rxjs';
import { ConfigService } from '@nestjs/config';

interface RequestOptions {
    headers?: Record<string, string>;
    config?: AxiosRequestConfig;
}

type HttpMethod = 'get' | 'post' | 'put' | 'delete';

@Injectable()
class HttpClientService {
    private readonly logger = new Logger(HttpClientService.name);
    private readonly defaultHeaders: Record<string, string>;
    private readonly retryCount: number;
    private readonly retryDelay: number;

    constructor(
        private readonly httpService: HttpService,
        private readonly config: ConfigService,
    ) {
        this.defaultHeaders = {
            'X-App-Source': 'nestjs-gateway',
            'X-Env': this.config.get<string>('NODE_ENV') || 'development',
        };
        this.retryCount = this.config.get<number>('http.retryCount') || 3;
        this.retryDelay = this.config.get<number>('http.retryDelay') || 1000;
    }

    private async request<T>(
        axiosInstance: AxiosInstance,
        method: HttpMethod,
        url: string,
        data?: any,
        options?: RequestOptions,
    ): Promise<T> {
        const headers = {
            ...this.defaultHeaders,
            ...options?.headers,
        };

        const config: AxiosRequestConfig = {
            method,
            url,
            headers,
            data,
            ...(options?.config || {}),
        };

        try {
            const response = await firstValueFrom(
                this.httpService
                    .request<T>({
                        ...config,
                        adapter: axiosInstance.defaults.adapter, 
                    })
                    .pipe(
                        retry({
                            count: this.retryCount,
                            delay: () => timer(this.retryDelay),
                        }),
                        map((res: AxiosResponse<T>) => res.data),
                        catchError((error: AxiosError) => {
                            this.logger.error(
                                `HTTP ${method.toUpperCase()} ${url} failed`,
                                error.stack || error.message,
                            );

                            if (error?.code === 'ECONNABORTED') {
                                throw new HttpException(
                                    'Request timeout',
                                    HttpStatus.GATEWAY_TIMEOUT,
                                );
                            }

                            if (!error?.response) {
                                throw new HttpException(
                                    'Service unavailable',
                                    HttpStatus.SERVICE_UNAVAILABLE,
                                );
                            }

                            throw new HttpException(
                                error.response.data || 'Remote service error',
                                error.response.status || HttpStatus.INTERNAL_SERVER_ERROR,
                            );
                        }),
                    ),
            );

            return response;
        } catch (error) {
            this.logger.error(
                `Final request failed: ${method.toUpperCase()} ${url}`,
                error.message,
            );
            throw error;
        }
    }

    private buildAxiosInstance(baseURL: string): AxiosInstance {
        return axios.create({
            baseURL: baseURL.replace(/\/+$/, ''),
        });
    }

    create(baseURL: string) {
        if (!baseURL) {
            throw new Error('HttpClientService.create(): baseURL is required');
        }

        const instance = this.buildAxiosInstance(baseURL);

        return {
            get: <T>(path: string, options?: RequestOptions) =>
                this.request<T>(instance, 'get', path, null, options),
            post: <T>(path: string, body: any, options?: RequestOptions) =>
                this.request<T>(instance, 'post', path, body, options),
            put: <T>(path: string, body: any, options?: RequestOptions) =>
                this.request<T>(instance, 'put', path, body, options),
            delete: <T>(path: string, options?: RequestOptions) =>
                this.request<T>(instance, 'delete', path, null, options),
        };
    }
}
export default HttpClientService;